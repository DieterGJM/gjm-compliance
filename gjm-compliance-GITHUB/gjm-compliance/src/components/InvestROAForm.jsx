...
const INSURERS = ['Sanlam', 'PPS', 'Old Mutual', 'BrightRock', 'Allan Gray', 'Coronation', 'Discovery', 'Momentum', 'Ninety One', 'Other']
...
export default function InvestROAForm({ data, onChange }) {
  ...
  const genSectionD = async (needType) => {
    ...
    try {
      const selected = needType === 'retirement' ? sD.retirementProductSelected : sD.savingsProductSelected
      const text = await generateSectionD(data, products, selected||'the selected product', needType)
      if (text) {
        if (needType==='retirement') setSd('retirementRationale', text)
        else setSd('savingsRationale', text)
      }
  ...
  const genSectionE = async () => {
    ...
    try {
      const text = await generateSectionE(data, products, data.occupation)
  ...
  const genSectionF = async () => {
    ...
    try {
      ...
      const text = await generateSectionF(allInsurers, selectedBenefits)
  ...
  return (
    <div>
      ...
      {products.map((p,i) => {
        ...
        try { availableBenefits = (p.insurer && p.insurer!=='Other') ? (getBenefitsForInsurer(p.insurer)||[]) : [] }
