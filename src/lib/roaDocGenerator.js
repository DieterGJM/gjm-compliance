...
export function buildROADoc(data) {
  ...
  // Pre-compute dynamic values to avoid IIFEs inside JSX-like expressions
  const sectionEProducts = data.selectedProductsText ||
    (() => {
      const chosen = Object.values(data.recommendations||{}).map(r=>r.selected).filter(Boolean)
      const unique = [...new Set(chosen)]
      return unique.length > 0
        ? unique.join(', ')
        : products.filter(p=>p.insurer&&p.product).map(p=>`${p.insurer} ${p.product}`).join(', ')
    })()

  const declinedProductsText = (sH.declinedProducts !== undefined && sH.declinedProducts !== '')
    ? sH.declinedProducts
  ...
  const children = [
    ...
    new Table({
      ...
      rows:[new TableRow({children:[
        ...
        new TableCell({children:[new Paragraph({children:[N(sectionEProducts)]})]}),
      ]}),
      new TableRow({children:[
    ...
    // Section H — clean rewrite with no inline IIFEs
    ...
    new Table({
      ...
      rows:[
        new TableRow({children:[
          ...
          new TableCell({children:[new Paragraph({children:[N(declinedProductsText)]})]}),
        ]}),
        new TableRow({children:[
