...
function sigBlock(showCO = false) {
  return [
    ...
    ...(showCO ? [
      sp(),
      new Paragraph({children:[B('FICA COMPLIANCE OFFICER or SENIOR MANAGEMENT (FOR ALL HIGH-RISK CLIENTS)')], spacing:{before:180}}),
...
function clientName(data) {
  const name = data.fullName || data.registeredName || '[Client Name]'
...
export function buildOnboardingDoc(data) {
  ...
  return makeDoc([
    ...
    hdr('9. ACCEPTANCE AND SIGN-OFF PROCESS'),
...
export function buildFNADoc(data) {
  ...
  return makeDoc([
    ...
    twoCol([
      ['Client Name', cName],
      ['Date of Birth', data.dob ? (() => { const d = new Date(data.dob+'T12:00:00'); return `${d.getFullYear()}/${String(d.getMonth()+1).padStart(2,'0')}/${String(d.getDate()).padStart(2,'0')}` })() : ''],
    ...
    hdr('INCOME ANALYSIS'),
    twoCol([
      ['Gross Annual Income Needed', fmt(data.grossAnnualIncomeNeeded || (Number(data.grossIncome || 0) * 12))],
