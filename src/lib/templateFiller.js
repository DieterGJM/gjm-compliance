...
// ── Apply signatures to any generated docx ───────────────────────────────
// Called after zip.file('word/document.xml', xml) in every download function
async function applySignatures(zip) {
  const advisorSig = (() => { try { return sessionStorage.getItem('gjm_sig_advisor') || '' } catch { return '' } })()
  if (!advisorSig) return zip  // no signature set — return unchanged

  // Embed image in zip
  injectSignatureImage(zip, advisorSig, 'rId15', 'sig_advisor')

  // Inject into document XML
  let xml = zip.file('word/document.xml').asText()

  // Replace signature line cells: find cells containing long underscore strings
  // and replace the underscores with the image + keep the label above
  const sigXml = sigImageXml('rId15', 1800000, 540000)  // ~50mm x 15mm

  // Strategy: find paragraphs that are pure underscore lines and replace them
  // These appear as: <w:p><w:r><w:t>___________...</w:t></w:r></w:p>
  xml = xml.replace(
    /(<w:p[^>]*>(?:<w:pPr>[\s\S]*?<\/w:pPr>)?<w:r>(?:<w:rPr>[\s\S]*?<\/w:rPr>)?<w:t[^>]*>_{10,}<\/w:t><\/w:r><\/w:p>)/g,
    `<w:p><w:r>${sigXml}</w:r></w:p>`
  )

  // Also handle signature cells in tables (ROA T12 R0 C2 = "Financial Advisor Signature")
  // Replace the empty cell after "Financial Advisor Signature" row
  // T12 R0: R0C0="Client's Signature", R0C2="Financial Advisor Signature"
  // T12 R1: names filled by setCell
  // The signature goes in R0 C2 — but there's no underscore there, just blank space
  // So we target the BLANK signature cell next to "Financial Advisor Signature" label
...
// ── Inject a signature image into a docx zip ─────────────────────────────
// sigB64: full data URL like "data:image/png;base64,ABC..."
// Returns modified zip with image embedded
function injectSignatureImage(zip, sigB64, relId, filename) {
  if (!sigB64 || !sigB64.includes(',')) return zip
  // Extract raw base64 (strip data:image/...;base64, prefix)
  const rawB64   = sigB64.split(',')[1]
  const mimeType = sigB64.split(';')[0].split(':')[1] || 'image/png'
  const ext      = mimeType.includes('png') ? 'png' : 'jpeg'
  const mediaPath = `word/media/${filename}.${ext}`
  ...
  const relEntry = `<Relationship Id="${relId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/${filename}.${ext}"/>`
...
// Build Word inline image XML (sized to fit in a signature line, ~40mm wide)
function sigImageXml(relId, widthEmu, heightEmu) {
  widthEmu  = widthEmu  || 1440000  // ~40mm
  heightEmu = heightEmu || 432000   // ~12mm
  return `<w:drawing><wp:inline distT="0" distB="0" distL="0" distR="0"><wp:extent cx="${widthEmu}" cy="${heightEmu}"/><wp:effectExtent l="0" t="0" r="0" b="0"/><wp:docPr id="1" name="sig"/><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="1" name="sig"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="${relId}" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="${widthEmu}" cy="${heightEmu}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing>`
}
...
export async function fillRATemplate(data, calc) {
  ...
  // ── Step 1: paragraph placeholders (run-merge approach) ──────────────
  xml = xml.replace(/<w:p[ >][\s\S]*?<\/w:p>/g, para => {
    ...
    const hasPlaceholder = /[\[[]/.test(fullText) ||
      ...
      fullText.includes('Interest Rate of 8%')
  ...
  // ── Step 2: Table 0 — inflation table (16 rows) ───────────────────────
  // Template has 16 [age][req income] placeholder rows
  // Fill with evenly-spaced years from current age to 65, always including age 65
  const inflTable = calc.inflationTable || []
  const total     = inflTable.length

  // Pick 15 evenly-spaced entries + always include the last (age 65)
  let indices = []
  if (total <= 15) {
    indices = inflTable.map((_, i) => i)
  } else {
    const step = (total - 1) / 14
    for (let i = 0; i < 15; i++) indices.push(Math.round(i * step))
    indices = [...new Set(indices)].sort((a, b) => a - b)
  }
  // Always include age 65 as the last row (slot 14), leave slot 15 blank
  if (indices[indices.length - 1] !== total - 1) {
    indices[indices.length - 1] = total - 1
  }

  indices.forEach((dataIdx, slot) => {
    if (slot >= 15) return  // max 15 rows of data (row 15 stays as placeholder)
    const row = inflTable[dataIdx]
    if (!row) return
    xml = setCell(xml, 0, slot, 0, String(row.age))
    xml = setCell(xml, 0, slot, 1, `R ${Math.round(row.income).toLocaleString('en-ZA')}`)
  })

  // ── Step 3: Table 1 — calculation blocks ─────────────────────────────
  // Template row indices (confirmed from blank template):
  // Current:    R2=yrs, R4=initial(monthly), R6=esc%, R8=rate%, R12=FV
  // Proposed 1: R17=yrs, R19=initial(monthly), R21=esc%, R23=rate%, R27=FV
  // Proposed 2: R31=yrs, R33=initial(monthly), R35=esc%, R37=rate%, R41=FV

  const curPrem = Number(data.currentPremium || 0)
  const blocks = [
    { r0:2,  r2:4,  r4:6,  r6:8,  r10:12, prem: curPrem,         esc: 10,  fv: calc.currentFV  || 0 },
    { r0:17, r2:19, r4:21, r6:23, r10:27, prem: calc.p2Premium   || 0, esc: 0,   fv: calc.p2FV       || 0 },
    { r0:31, r2:33, r4:35, r6:37, r10:41, prem: calc.p3Premium   || 0, esc: 5,   fv: calc.p3FV       || 0 },
  ]

  blocks.forEach(({ r0, r2, r4, r6, r10, prem, esc, fv }) => {
    xml = setCell(xml, 1, r0,  1, String(calc.yrs || ''))
    xml = setCell(xml, 1, r2,  1, prem ? `R ${Math.round(prem).toLocaleString('en-ZA')}` : 'R 0')
    xml = setCell(xml, 1, r4,  1, `${esc}%`)
    xml = setCell(xml, 1, r6,  1, `${intPct}%`)
    xml = setCell(xml, 1, r10, 1, fv ? `R ${Math.round(fv).toLocaleString('en-ZA')}` : 'R 0')
  })
