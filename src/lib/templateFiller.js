...
// Populates FNA_Calculator.xlsx with client data, keeping all Excel formulas
// Input cells: D7=DOB, E9=retAge, E12=grossIncome, E22=interestRate,
//              E23=fundValue, E36=penalties, B{age-16}=reqIncome
// Override cells: E8,E10,E13,E16,E17,E18,E19,E24,E26,E27,E30-E34 (correct calcs)
// Liability sheet: G7-G39
// ══════════════════════════════════════════════════════════════════════════
export async function fillFNAExcel(data, calc) {
  // Load the Excel template as a binary blob
  const b64    = fnaTemplate
  const binary = atob(b64)
  const bytes  = new Uint8Array(binary.length)
  for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i)

  // We use SheetJS (xlsx) to manipulate the workbook in-browser
  const XLSX = await import('xlsx')
  const wb   = XLSX.read(bytes, { type: 'array', cellStyles: true })

  const fna  = wb.Sheets['FNA Calculator']
  const liab = wb.Sheets['Liability Calculator']

  // Helper to set a cell value
  const set = (sheet, addr, value, fmt) => {
    sheet[addr] = { v: value, t: typeof value === 'number' ? 'n' : 's' }
    if (fmt) sheet[addr].z = fmt
  }
  ...
  // DOB as Excel serial number
  if (data.dob) {
    const d = new Date(data.dob + 'T12:00:00')
    const epoch = new Date(1899, 11, 30)
    const serial = Math.round((d - epoch) / 86400000)
    set(fna, 'D7', serial, 'yyyy/mm/dd')
    fna['D7'].t = 'n'
  }

  set(fna, 'E9',  calc.retirementAge || 65)
  set(fna, 'E12', Number(data.grossIncome || 0))
  set(fna, 'E22', rate)
  set(fna, 'E23', Number(data.currentFundValue || 0))
  set(fna, 'E36', Number(data.penalties || 0))
  set(fna, 'E14', Number(data.grossIncome || 0) * 12)

  // Required income at client's current age row
  // Row = age - 16 (A6=22, so age X is at row X-16)
  const reqRow  = age - 16
  const reqAddr = \`B\${reqRow}\`
  set(fna, reqAddr, Number(data.reqIncome || 0))

  // ── Override calculated cells with correct values ─────────────────────
  const tax  = calc.tax || {}
  const fees = calc.fees || {}

  set(fna, 'E8',  age)
  set(fna, 'E10', yrs)
  set(fna, 'E13', tax.netMonthly || 0)
  set(fna, 'E16', tax.bracket || '')
  set(fna, 'E17', tax.maxRAmonthly || 0)
  set(fna, 'E18', tax.sarsRebate || 0)
  set(fna, 'E19', tax.netCostOfContribution || 0)
  set(fna, 'E24', Math.round(calc.futureInvestmentValue || 0))
  set(fna, 'E26', Math.round(calc.totalFundRequired || 0))
  set(fna, 'E27', Math.round(calc.shortfall || 0))
  set(fna, 'E30', Math.round(fees.allanGrayFee || 0))
  set(fna, 'E31', Math.round(fees.existingFee || 0))
  set(fna, 'E33', Math.round(fees.feeSavings || 0))
  set(fna, 'E34', Math.round(fees.feeSavingsToRetirement || 0))

  // ── Liability Calculator sheet ─────────────────────────────────────────
  const grossAnnual = Number(data.grossIncome || 0) * 12

  set(liab, 'G7',  Number(data.mortgages         || 0))
  set(liab, 'G8',  Number(data.loans             || 0))
  set(liab, 'G9',  Number(data.finalExpenses      || 0))
  set(liab, 'G11', Number(data.educationFund      || 0))
  set(liab, 'G14', Number(data.childCare          || 0))
  set(liab, 'G16', Number(data.otherCashNeeds     || 0))
  set(liab, 'G22', grossAnnual)
  set(liab, 'G23', Number(data.partnerIncome      || 0))
  set(liab, 'G24', Math.max(0, grossAnnual - Number(data.partnerIncome || 0)))
  set(liab, 'G25', Math.max(0, grossAnnual - Number(data.partnerIncome || 0)))
  set(liab, 'G26', Number(data.assumedReturn      || 0))
  set(liab, 'G27', 0)  // =0 when assumed return is 0
  set(liab, 'G33', Number(data.cashAssets         || 0))
  set(liab, 'G34', Number(data.stocksBonds        || 0))
  set(liab, 'G35', Number(data.principalResidence || 0))
  set(liab, 'G36', Number(data.secondaryResidence || 0))
  set(liab, 'G37', Number(data.totalLifeInsurance || 0))
  set(liab, 'G38', Number(data.businessAssets     || 0))
