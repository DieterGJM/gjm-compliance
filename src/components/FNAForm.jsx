...
export default function FNAForm({ data, onChange }) {
  ...
  const interestRate  = (Number(data.interestRate) > 1 ? Number(data.interestRate) / 100 : Number(data.interestRate)) || EXCEL.DEFAULT_INTEREST_RATE
    ...
    <div>
      ...
      <div className="form-grid">
        <div className="form-group">
          ...
          <input type="number"
            value={data.grossAnnualIncomeNeeded || (Number(data.grossIncome || 0) * 12) || ''}
            onChange={set('grossAnnualIncomeNeeded')} placeholder="Auto: gross × 12" />
        </div>
        <div className="form-group">
