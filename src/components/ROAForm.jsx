...
import { generateNeedsObjectives, generateFinancialSituation, generateProductKnowledge, generateSectionD, generateSectionE, generateSectionF, generateDeclinedReason } from '../lib/roaGenerator'
...
export default function ROAForm({ data, onChange }) {
  ...
  // Auto-initialise Section B needs table + auto-populate amounts from entered data
  useEffect(() => {
    const reviewDate = oneYearFromToday()
    const existing = data.needsTable || {}
    const needsUpdate = Object.keys(existing).length === 0

    // Auto-derive amounts from available data fields
    const gross   = Number(data.grossIncome || 0)
    const reqRet  = Number(data.reqIncome   || 0)
    const debt    = Number(data.currentDebt || 0)
    const recs    = data.recommendations    || {}
    const hasWill = (data.otherInfo || '').toLowerCase().includes('will')

    const autoAmounts = {
      emergency:  gross    ? `R${(gross*3).toLocaleString('en-ZA')} (3× monthly salary)` : '',
      death:      recs.death?.life_cover_required
                    ? `R${Number(recs.death.life_cover_required).toLocaleString('en-ZA')}`
                    : debt ? `R${debt.toLocaleString('en-ZA')}` : '',
      disability: recs.disability?.income_required
                    ? `R${Number(recs.disability.income_required).toLocaleString('en-ZA')} p/m`
                    : gross ? `R${gross.toLocaleString('en-ZA')} p/m` : '',
      trauma:     recs.trauma?.cover_required
                    ? `R${Number(recs.trauma.cover_required).toLocaleString('en-ZA')}`
                    : recs.trauma?.cover_considered
                    ? `R${Number(recs.trauma.cover_considered).toLocaleString('en-ZA')}` : '',
      retirement: reqRet ? `R${reqRet.toLocaleString('en-ZA')} p/m at retirement` : '',
      will:       hasWill ? 'Yes — client has an updated will' : '',
      savings: '', assets: '', health: '', other: '',
    }
    ...
    if (needsUpdate) {
      ...
      NEEDS_CONFIG.forEach(n => {
        defaultNeeds[n.key] = {
          ...
          amount:     autoAmounts[n.key] || '',
        }
      })
      onChange({ ...data, needsTable: defaultNeeds, age: derivedAge })
    } else {
      // Update amounts only where not yet manually entered
      const updated = { ...existing }
      let changed = false
      Object.keys(autoAmounts).forEach(key => {
        if (autoAmounts[key] && !existing[key]?.amount) {
          updated[key] = { ...(existing[key]||{}), amount: autoAmounts[key] }
          changed = true
        }
      })
      if (changed) onChange({ ...data, needsTable: updated })
    }
  }, [
    data.grossIncome, data.reqIncome, data.currentDebt, data.otherInfo,
    JSON.stringify(data.recommendations?.death),
    JSON.stringify(data.recommendations?.disability),
    JSON.stringify(data.recommendations?.trauma),
  ])
  ...
  // Auto-fill declined reasons from declined products
  useEffect(() => {
    const declined = sH.declinedProducts
    if (!declined || !declined.trim()) return
    // Only auto-fill if reasons is empty or was previously auto-filled (not manually typed)
    if (sH.declinedReasons && !sH._reasonsAutoFilled) return
    // Derive the recommended product from recs
    const recommended = [...new Set(
      Object.values(recs).map(r => r.selected?.split(' — ')[0]).filter(Boolean)
    )].join(' & ')
    const reason = recommended
      ? `Client has a preference for ${recommended} over ${declined}`
      : `Client has a preference for the recommended product over ${declined}`
    onChange({ ...data, sectionH: { ...sH, declinedReasons: reason, _reasonsAutoFilled: true } })
  }, [sH.declinedProducts])

  // AI handlers — Section A
  const genNeedsObjectives = async () => {
    setGen('needsObjectives', true)
    try {
      const text = await generateNeedsObjectives({ ...data, age: derivedAge, products })
      if (text) setV('needsObjectives', text)
    } catch(e) { console.error(e) }
    setGen('needsObjectives', false)
  }

  const genFinancialSituation = async () => {
    setGen('financialSituation', true)
    try {
      const text = await generateFinancialSituation({ ...data, age: derivedAge })
      if (text) setV('financialSituation', text)
    } catch(e) { console.error(e) }
    setGen('financialSituation', false)
  }

  const genProductKnowledge = async () => {
    setGen('productKnowledge', true)
    try {
      const text = await generateProductKnowledge({ ...data, age: derivedAge }, products)
      if (text) setV('productKnowledge', text)
    } catch(e) { console.error(e) }
    setGen('productKnowledge', false)
  }
  ...
  const genSectionF = async () => {
    ...
    try {
      ...
      // Pass selected benefits so Section F uses exact product notes for chosen benefits
      const selectedBenefits = products.flatMap(p =>
        (p.selectedBenefits||[]).map(id => ({ insurerName: p.customInsurer||p.insurer, benefitId: id }))
      )
      const text = await generateSectionF(insurers, selectedBenefits)
  ...
  return (
    <div>
      ...
      <div className="form-grid">
        <div className="form-group span-2">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.4rem'}}>
            <label style={{margin:0}}>Client's Needs and Objectives</label>
            <AIButton onClick={genNeedsObjectives} loading={generating.needsObjectives} label="Auto-Generate" />
          </div>
          <textarea value={data.needsObjectives||''} onChange={set('needsObjectives')} style={{minHeight:'55px'}}
            placeholder="Click Auto-Generate or type — must be FSCA-compliant reason for cover, not just 'reduce premiums'" />
        </div>
        <div className="form-group span-2">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.4rem'}}>
            <label style={{margin:0}}>Financial Situation</label>
            <AIButton onClick={genFinancialSituation} loading={generating.financialSituation} label="Auto-Generate" />
          </div>
        ...
        <div className="form-group span-2">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.4rem'}}>
            <label style={{margin:0}}>Product Knowledge and Experience</label>
            <AIButton onClick={genProductKnowledge} loading={generating.productKnowledge} label="Auto-Generate" />
          </div>
          <textarea value={data.productKnowledge||''} onChange={set('productKnowledge')} style={{minHeight:'50px'}}
            placeholder="Click Auto-Generate for profession-specific product knowledge statement" />
        </div>
        <div className="form-group span-2">
          <label>Other Information</label>
          <div style={{display:'flex',flexWrap:'wrap',gap:'0.4rem',marginBottom:'0.5rem'}}>
            {['Client has a will','Client has a tax practitioner','Estate planning in place',
              'Client is a PPS member','Client has existing RA','Client has existing life cover',
              'Client has medical aid','Trust in place'].map(opt => {
              const checked = (data.otherInfoItems||[]).includes(opt)
              return (
                <label key={opt} style={{display:'flex',alignItems:'center',gap:'0.3rem',
                  padding:'0.3rem 0.65rem',borderRadius:'20px',cursor:'pointer',fontSize:'0.78rem',
                  background:checked?'var(--gold)':'var(--white)',
                  border:checked?'1.5px solid var(--gold)':'1.5px solid var(--border-light)',
                  fontWeight:checked?700:400,color:checked?'var(--black)':'var(--slate)'}}>
                  <input type="checkbox" checked={checked} style={{width:'auto',accentColor:'var(--gold)'}}
                    onChange={()=>{const c=data.otherInfoItems||[];setV('otherInfoItems',checked?c.filter(x=>x!==opt):[...c,opt])}} />
                  {opt}
                </label>
              )
            })}
          </div>
          <input value={data.otherInfo||''} onChange={set('otherInfo')}
            placeholder="Additional notes (optional)" style={{fontSize:'0.82rem'}} />
        </div>
      </div>
      ...
      {products.map((p,i) => {
        ...
        try {
          availableBenefits = (p.insurer && p.insurer !== 'Other') ? (getBenefitsForInsurer(p.insurer) || []) : []
        ...
        return (
          <div key={i} style={{background:'var(--off-white)',border:'1px solid var(--border-light)',borderRadius:'8px',padding:'0.85rem',marginBottom:'0.75rem'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1.5fr 1fr 0.5fr auto',gap:'0.75rem',alignItems:'end',marginBottom: availableBenefits.length ? '0.75rem' : 0}}>
              ...
              <div className="form-group" style={{margin:0}}>
                ...
                <input value={p.product} onChange={e=>setProduct(i,'product',e.target.value)} placeholder="e.g. TopCover + Income Protector" />
              </div>
              <div className="form-group" style={{margin:0}}>
            ...
            {availableBenefits.length > 0 && (
              <div>
                <div style={{fontSize:'0.74rem',fontWeight:600,color:'var(--slate)',textTransform:'uppercase',letterSpacing:'0.04em',marginBottom:'0.4rem'}}>
                  Benefits included — select all that apply:
                </div>
                  ...
                  {availableBenefits.map(b => {
                    ...
                    return (
                      <button key={b.id} type="button"
                        onClick={() => {
                          const curr = p.selectedBenefits || []
                          setProduct(i, 'selectedBenefits', selected ? curr.filter(id=>id!==b.id) : [...curr, b.id])
                        }}
                        style={{
                          padding:'0.3rem 0.7rem',fontSize:'0.75rem',borderRadius:'20px',cursor:'pointer',
                          background:   selected ? 'var(--gold)' : 'var(--white)',
                          color:        selected ? 'var(--black)' : 'var(--slate)',
                          border:       selected ? '1.5px solid var(--gold)' : '1.5px solid var(--border-light)',
                          fontWeight:   selected ? 700 : 400,
                          transition:   'all 0.15s',
                        }}>
                        {b.types.includes('life') ? '🛡' : b.types.includes('trauma') ? '❤️' : b.types.includes('income') ? '💼' : '🔒'} {b.name}
                      </button>
                    )
                  })}
                </div>
                {(p.selectedBenefits||[]).length > 0 && (
                  <div style={{marginTop:'0.4rem',fontSize:'0.73rem',color:'var(--slate-light)',fontStyle:'italic',lineHeight:'1.5'}}>
                    {availableBenefits.filter(b=>(p.selectedBenefits||[]).includes(b.id)).map(b=>b.shortDesc).join(' · ')}
                  </div>
                )}
              </div>
            )}
      ...
      <div className="form-grid">
        ...
        <div className="form-group span-2">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'0.35rem'}}>
            <label style={{margin:0}}>2. Reasons the client elected not to accept the recommendations</label>
            <AIButton label="Generate" loading={generating.declinedReason} onClick={async () => {
              setGen('declinedReason', true)
              try {
                const text = await generateDeclinedReason({...data}, sH.declinedProducts||'', products)
                if (text) setSH('declinedReasons', text)
              } catch(e) {}
              setGen('declinedReason', false)
            }} />
          </div>
          <input value={sH.declinedReasons||''} onChange={e=>{ setSH('declinedReasons',e.target.value); setSH('_reasonsAutoFilled',false) }}
