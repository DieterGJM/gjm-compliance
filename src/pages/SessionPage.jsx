...
import {
  downloadFilledOnboarding, downloadFilledOngoing,
  downloadFilledTransactional, downloadFilledROA, downloadFilledRA,
  downloadFilledFNA, downloadFilledInvestROA
} from '../lib/templateFiller'
import { calcRA } from '../lib/calculations'
import { downloadROADocx } from '../lib/roaDocGenerator'
import ROAForm from '../components/ROAForm'
import InvestROAForm from '../components/InvestROAForm'
...
const DOC_MAP = {
  ...
  investroa: { label: 'Invest ROA', build: null },
}
...
export default function SessionPage() {
  ...
  const handleDownload = async (docId, format) => {
    ...
    try {
      ...
      if (docId === 'roa') {
        await downloadFilledROA(data)
      } else if (docId === 'investroa') {
        await downloadFilledInvestROA(data)
      } else if (format === 'docx') {
        // Use actual template files for compliance documents
        if (docId === 'onboarding') {
          await downloadFilledOnboarding(data)
        } else if (docId === 'ongoing') {
          await downloadFilledOngoing(data)
        } else if (docId === 'transactional') {
          await downloadFilledTransactional(data)
        } else if (docId === 'fna') {
          // Download FNA as Excel (populated FNA_Calculator.xlsx)
          const calc = { ...calcRA(data), tax: calcRA(data).tax || {}, fees: calcRA(data).fees || {} }
          await downloadFilledFNA(data, calc)
        } else if (docId === 'ra') {
          // Use actual RA Calculation template
          const calc = calcRA(data)
          await downloadFilledRA(data, calc)
        } else {
    ...
    <div>
      ...
      {step === 'forms' && (
        <div>
          ...
          {selectedDocs.includes('investroa') && (
          <div className="card" style={{ marginBottom: '1.5rem' }}>
            <div className="card-header"><h3>Investment ROA</h3></div>
            <div className="card-body">
              <InvestROAForm data={formData.investroa || {}} onChange={setDocData('investroa')} />
            </div>
          </div>
        )}
